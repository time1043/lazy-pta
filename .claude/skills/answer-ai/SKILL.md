...
> **CRITICAL: Only operate on `answer-ai.json`. `answer-ai-submit.json` is INVISIBLE to this skill — never read, write, or modify it.** The submit file is generated by the `answerAiSubmit` service, not by this skill.
...
- **Only generate `answer-ai.json`. Never generate `answer-ai-submit.json`** — the submit file is generated by the `answerAiSubmit` service, not by this skill.
...
After generating or updating `answer-ai.json`, remind the user to run the following commands:

```
pnpm data:down:ai-submit
pnpm down # or pnpm dev
pnpm pta:answer-ai-submit
```

- `pnpm data:down:ai-submit` — generates `answer-ai-submit.json` from `answer-ai.json`
- `pnpm down` — downloads/refreshes data (or `pnpm dev` to start dev server)
- `pnpm pta:answer-ai-submit` — submits answers to PTA

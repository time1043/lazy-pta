import { isProblemType } from '@/libs/types/problem-type';
import { AnswerUrl, IdsAnswerUrl, IdsQuestionUrl, IdsUrl, QuestionUrl } from '@/libs/types/url';
...
export function questionUrlMatcher(urlStr: string): MatchResult<QuestionUrl> {
...
export function answerUrlMatcher(urlStr: string): MatchResult<AnswerUrl> {
  const url = getUrlObj(urlStr);
  if (!url) return null;

  const matcher = /^\/api\/exams\/([^/]+)\/problem-sets\/([^/]+)\/last-submissions$/;
  const match = url.pathname.match(matcher);
  const problemType = url.searchParams.get('problem_type');
...
export function idsUrlMatcher(urlStr: string): MatchResult<IdsUrl> {
  const url = getUrlObj(urlStr);
  if (!url) return null;
...
export function idsQuestionUrlMatcher(urlStr: string): MatchResult<IdsQuestionUrl> {
  const url = getUrlObj(urlStr);
  if (!url) return null;
  ...
  return match ? { urlStr, problemSetId: match[1], problemSetProblemId: match[2] } : null;
}

export function idsAnswerUrlMatcher(urlStr: string): MatchResult<IdsAnswerUrl> {
  const url = getUrlObj(urlStr);
  if (!url) return null;
